...
# === UTILS ===
def get_model_step(pipeline):
    """Finds the RandomForest step from any sklearn pipeline (flexible name)."""
    for step_name in pipeline.named_steps:
        step = pipeline.named_steps[step_name]
        if hasattr(step, 'predict_proba') and 'RandomForest' in str(type(step)):
            return step
    raise ValueError("No RandomForest model found in pipeline!")
...
def get_shap_values(pipeline, X):
    # Folosește cache locală pentru .npy, ca să nu recalculezi mereu
    if os.path.exists(SHAP_VALUES_PATH):
        shap_values = np.load(SHAP_VALUES_PATH, allow_pickle=True)
        explainer = shap.TreeExplainer(get_model_step(pipeline))
    else:
        explainer = shap.TreeExplainer(get_model_step(pipeline))
