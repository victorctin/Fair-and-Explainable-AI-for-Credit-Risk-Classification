...
def load_and_sync():
    ...
    # Synchronize all datasets on common indices
    common_idx = X_test_final.index.intersection(y_test.index).intersection(df_test_fairness.index)
    X_sync = X_test_final.loc[common_idx].reset_index(drop=True)
    y_sync = y_test.loc[common_idx].reset_index(drop=True)
    df_sync = df_test_fairness.loc[common_idx].reset_index(drop=True)

    # Save synced versions for reproducibility
...
def get_shap_values(_pipeline, _X):
    # Extract RandomForest from any pipeline
    def get_estimator(pipeline):
        ...
        if hasattr(pipeline, 'predict'):  # fallback for classifier
            return pipeline
...
# === LEGEND FOR PROTECTED ATTRIBUTES ===
def show_legend():
    st.markdown("""
